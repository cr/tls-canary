...
class WorkerReader(Thread):
    ...
    def run(self):
        ...
        logger.debug('Reader thread started for worker %s' % self.worker)
        ...
        for line in iter(self.worker.worker_thread.stdout.readline, b''):
            ...
            try:
                ...
                if line.startswith("JavaScript error:"):
                    logger.error("JS error from worker %s: %s" % (self.worker, line))
                elif line.startswith("JavaScript warning:"):
                    logger.warning("JS warning from worker %s: %s" % (self.worker, line))
                else:
                    logger.critical("Invalid output from worker %s: %s" % (self.worker, line))
        self.worker.worker_thread.stdout.close()
        logger.debug('Reader thread finished for worker %s' % self.worker)
...
class WakeupHandler(msg.MessagingThread):
    ...
    def run(self):
        ...
        logger.debug('Wakeup handler thread started for worker %s' % self.worker)
        # This thread will automatically end when worker's stdout is closed
        while self.worker.is_running():
            try:
                response = self.worker.response_queue.get(timeout=0.05)
                cmd = self.worker.get_pending(response)
                logger.error("Command for response %s is %s" % (response, cmd))
        ...
        logger.debug('Wakeup handler thread finished for worker %s' % self.worker)
        del self.worker  # Breaks cyclic reference
...
class XPCShellWorker(object):
        ...
        with self.__write_lock:
            try:
                ...
                if set_pending:
                    ...
                    self.set_pending(cmd, response_queue)
    ...
    def set_pending(self, cmd, queue):
        """Register a Command as pending for responses"""
        with self.__pending_lock:
            self.__pending[cmd.id] = queue
        logger.critical("Now %s is pending: %s [%s]" % (cmd, self.__pending, self))

    def get_pending(self, response):
        """Return pending Command associated with a Response's ID"""
        logger.critical("Getting pending %s from %s [%s]" % (response, self.__pending, self))
        try:
    ...
    def del_pending(self, cmd):
        ...
        logger.critical("Deleting %s from pending %s [%s]" % (cmd, self.__pending, self))
        with self.__pending_lock:
